function countdownTimer() { //creates a function for whenever the start button is clicked
 ...
 //I created a for loop below, because I got sick and tired of seeing the timer run off of 85 lines of code, as opposed to 18
    for (var ct=10;ct>=1;ct--) { //creates a for loop. Critera are ct is set equal to 10, before each iteration, check to see if ct is larger than or equal to 1, and subtract 1 from ct after each iteration 
        (function(currTime) { //creates a function that uses the parameter currTime (which will be used later)
            setTimeout(function(){ //creates a function that can be set to only happen after a certain time
                document.getElementById("countdownStatus").innerHTML = (currTime * 5); //sets the countdownStatus in the linked HTML file to change to the parameter currTime, times the number 5
                console.log(currTime * 5 + " seconds");}, //prints the parameter currTime times the number 5 to the console, followed by 'seconds'
    ...
    setTimeout(function(){ //creates another function that can be set happen after a certain time
        //stuff in here happens after timeout
        document.getElementById("countdownStatus").innerHTML = "Blast Off!!"; //prints Blast Off!! to the countdownStatus in the linked HTML doc
        console.log("Changed to Blastoff!!"); //prints Changed to Blastoff!! to the console
    }, 50000) //tells the function to only run after 50 seconds, when the countdown timer would have gotten to zero
}
